...
function hydrateTask(dbTask = {}) {
  const byLabel = { low: 0, medium: 1, high: 2, urgent: 3 };
  const byId = { 1: 0, 2: 1, 3: 2, 4: 3 };

  let uiPriority = byLabel[String(dbTask.priority || "").toLowerCase()];
  ...
  if (uiPriority == null && Number.isFinite(dbTask.priority_id)) {
    uiPriority = byId[Number(dbTask.priority_id)];
  }
  ...
  const etaMin = Number.isFinite(etaSecRaw) ? Math.round(etaSecRaw / 60) : undefined;
...
function QuickAdd({ onAdd }) {
  ...
  return (
    <form onSubmit={submit} className="flex items-center gap-2 rounded-2xl border border-platinum bg-white px-3 py-2 shadow-sm">
      ...
      <input
        ...
        disabled={pending}
      />
      <button className="rounded-xl bg-brand px-3 py-1 text-sm text-white disabled:opacity-60" disabled={pending || !text.trim()}>
        {pending ? "Adding…" : "Add"}
      </button>
  ...
  return (
    <motion.div
      ...
      className={cx(
        ...
        inProgress ? "border-amber-300 bg-amber-50" : done ? "border-gray-300 bg-gray-100" : "border-platinum bg-white"
      )}
      onClick={(e) => {
    ...
    >
      ...
      <div className="min-w-0">
        <div className={cx("truncate font-medium text-oxford-blue", done && "line-through text-gray-400 opacity-70 font-normal")}>
          <HoverTitle title={task.title} subtitle={proj ? `Project: ${proj.name}` : undefined} />
          {inProgress && paused && (
        ...
        <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-500">
          ...
          <span className="inline-flex items-center gap-1">
            <TagIcon size={14} className={task.tags?.length ? "text-oxford-blue" : "text-gray-400"} aria-hidden="true" />
            {task.tags?.length ? (
              <span className="flex flex-wrap gap-1">
                {task.tags.map((t, i) => (
                  <span key={`${t}-${i}`} className="rounded-full bg-platinum px-2 py-0.5 text-[10px]">
                    #{t}
                  </span>
                ))}
              </span>
          ...
          <span className="inline-flex items-center gap-1">
            <Clock3 size={14} />
            {task.status === "done" ? formatDuration(task.effective_sec ?? 0) : `ETA: ${task.etaMin ?? "—"}min`}
          </span>
      ...
      <div className="flex items-center gap-2">
        {inProgress ? (
          ...
          <button
            ...
            className={cx(
              ...
              done ? "border-platinum bg-white text-gray-400" : "border-platinum bg-white hover:bg-platinum/60"
            )}
        ...
        {!done && (
          <button
            onClick={(e) => {
              e.stopPropagation();
              inProgress && onStop(task);
            }}
            disabled={!inProgress}
            className={cx(
              "inline-flex items-center gap-2 rounded-lg border px-2 py-1 text-xs",
              inProgress ? "border-red-200 bg-red-50 text-red-700 hover:bg-red-100" : "border-platinum bg-white text-gray-400"
            )}
            title="Stop"
          >
            <Square size={16} /> Stop
        ...
        <button
          onClick={(e) => {
            e.stopPropagation();
            !inProgress && onToggleDone(task.id);
          }}
          className={cx(
            "inline-flex items-center gap-2 rounded-lg border px-2 py-1 text-xs",
            done ? "border-emerald-300 bg-emerald-50 text-emerald-700" : "border-platinum bg-white hover:bg-platinum/60"
          )}
        >
...
export default function TodoView({ onPickTask }) {
  ...
  async function stopTask(task) {
    ...
    try {
      ...
      const justStopped = tasks.find((t) => t.id === task.id);
  ...
  async function toggleDone(id) {
    try {
      ...
      setTasks((prev) => prev.map((x) => (x.id === id ? hydrateTask(updated) : x)));
  ...
  async function bulkDelete() {
    try {
      ...
      console.error(e);
      setErr(e?.message || "Failed to delete all selected tasks");
      toast("Failed to delete all selected tasks", "error");
    }
  }
  ...
  const pageVisible = useMemo(() => visible.slice(pageStart, pageEnd), [visible, pageStart, pageEnd]);
  ...
  // selection based on current page
  const pageVisibleIds = useMemo(() => new Set(pageVisible.map((t) => t.id)), [pageVisible]);
  const anySelectedVisible = [...selected].some((id) => pageVisibleIds.has(id));
  const allVisibleSelected = pageVisible.length > 0 && pageVisible.every((t) => selected.has(t.id));

  function toggleSelectAllVisible() {
    setSelected((prev) => {
      const next = new Set(prev);
      if (allVisibleSelected) pageVisible.forEach((t) => next.delete(t.id));
      else pageVisible.forEach((t) => next.add(t.id));
      return next;
    });
  }
    ...
    <div className="min-w-0 max-w-full">
      ...
      {anySelectedVisible && (
        <div className="mt-3 flex items-center justify-between rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
          ...
          <div className="flex items-center gap-2">
            <button onClick={bulkMarkDone} className="rounded-lg bg-amber-900/10 px-3 py-1 hover:bg-amber-900/15">Mark done</button>
            <button onClick={bulkDelete} className="rounded-lg bg-amber-900/10 px-3 py-1 hover:bg-amber-900/15">Delete</button>
            <button onClick={clearSelection} className="rounded-lg bg-amber-900/10 px-3 py-1 hover:bg-amber-900/15">Clear</button>
          </div>
        </div>
      )}
      ...
      <div className="mt-3 flex items-center justify-between text-xs text-gray-600">
        <div className="flex items-center gap-2">
          ...
          />
          <span>
            {pageVisible.length} / {totalVisible} task{totalVisible === 1 ? "" : "s"}
          </span>
        </div>
      </div>
